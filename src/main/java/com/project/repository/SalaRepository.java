...
public interface SalaRepository extends JpaRepository<Sala, Long>{
	...
	@Query("select s from Sala s left join Operacija p on s.id = p.sala.id where (p.datum != ?1 or p.datum is null) and (p.vremeOd!= ?2 or p.vremeOd is null) ")
	List<Sala> getSaleOp(LocalDate datum, LocalTime vreme);
